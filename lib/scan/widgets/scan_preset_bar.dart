      ...
      child: Column(
        ...
        children: <Widget>[
          SingleChildScrollView(
            ...
            child: Row(
              children: <Widget>[
                for (final MapEntry<PdfScanPreset, String> entry in _labels.entries)
                  Padding(
                    ...
                    child: ChoiceChip(
                      ...
                      onSelected: enabled
                          ? (_) => bloc.add(ScanPresetSelected(entry.key, applyToAll: false))
                          : null,
                    ),
                  ),
              ],
          ...
          Row(
            children: <Widget>[
              ...
              GestureDetector(
                onTapDown: enabled ? (_) => bloc.add(const ScanComparisonToggled(true)) : null,
                onTapUp: enabled ? (_) => bloc.add(const ScanComparisonToggled(false)) : null,
                onTapCancel: enabled ? () => bloc.add(const ScanComparisonToggled(false)) : null,
                child: Chip(
                  avatar: const Icon(Icons.compare_outlined, size: 18),
                  label: Text(state.isComparingOriginal ? 'Showing original' : 'Hold to compare'),
                ),
              ),
            ],
          ),
